...
class Follow(db.Model):
    ...
    user_being_followed_id = db.Column(#1 #2 #1
        db.Integer,
        db.ForeignKey('users.id', ondelete="cascade"),
        primary_key=True,
    )

    user_following_id = db.Column(#2 #1 #7
        db.Integer,
        db.ForeignKey('users.id', ondelete="cascade"),
        primary_key=True,
    )
...
class User(db.Model):
    ...
    followers = db.relationship(
        "User",
        secondary="follows",
        primaryjoin=(Follow.user_being_followed_id == id), # all records that have user in being followed
        secondaryjoin=(Follow.user_following_id == id), # get info on these users from the matching record
        backref="following",
    )
